...
def get_custom(engine: Engine, columns:str | list[str], table: str) -> pd.DataFrame:
...
def get_accounts(engine: Engine, old_user_ids: pd.Series | None = None) -> pd.DataFrame:
    if old_user_ids is not None:
        ...
        return pd.read_sql(f"SELECT AccountID, OldUserID FROM app.Accounts WHERE OldUserID IN {acc_ids}", engine)
    return pd.read_sql("SELECT AccountID, OldUserID FROM app.Accounts", engine)
...
def get_locations(engine: Engine, old_location_ids: pd.Series | None = None) -> pd.DataFrame:
    if old_location_ids is not None:
        ...
        return pd.read_sql(f"SELECT LocationID, OldLocationID FROM app.Locations WHERE OldLocationID IN {loc_ids}", engine)
    return pd.read_sql("SELECT LocationID, OldLocationID FROM app.Locations", engine)
...
def get_users(engine: Engine, old_subuser_ids: pd.Series | None = None) -> pd.DataFrame:
    if old_subuser_ids is not None:
        ...
        return pd.read_sql(f"SELECT ID, OldID FROM app.AspNetUsers WHERE UserType='User' AND OldID IN {user_ids}", engine)
    return pd.read_sql("SELECT ID, OldID FROM app.AspNetUsers WHERE UserType='User'", engine)



def get_customers(engine: Engine, old_customer_ids: pd.Series | None = None) -> pd.DataFrame:
    if old_customer_ids is not None:
        cust_ids = (0,0) + tuple(old_customer_ids.values.tolist())
        return pd.read_sql(f"SELECT ID, OldID FROM app.AspNetUsers WHERE UserType='Customer' AND OldID IN {cust_ids}", engine)
    return pd.read_sql("SELECT ID, OldID FROM app.AspNetUsers WHERE UserType='Customer'", engine)



def get_makes(engine: Engine, old_make_ids: pd.Series | None = None) -> pd.DataFrame:
    if old_make_ids is not None:
        make_ids = (0,0) + tuple(old_make_ids.values.tolist())
        return pd.read_sql(f"SELECT MakeID, OldMakeID FROM app.Makes WHERE OldMakeID IN {make_ids}", engine)
    return pd.read_sql("SELECT MakeID, OldMakeID FROM app.Makes", engine)


def get_orders(engine: Engine, old_order_ids: pd.Series | None = None) -> pd.DataFrame:
    if old_order_ids is not None:
        order_ids = (0,0) + tuple(old_order_ids.values.tolist())
        return pd.read_sql(f"SELECT OrderID, OldOrderID FROM app.Orders WHERE OldOrderID IN {order_ids}", engine)
    return pd.read_sql("SELECT OrderID, OldOrderID FROM app.Orders", engine)

def get_cars(engine: Engine, old_car_ids: pd.Series | None = None) -> pd.DataFrame:
    if old_car_ids is not None:
        car_ids = (0,0) + tuple(old_car_ids.values.tolist())
        return pd.read_sql(f"SELECT CarID, OldCarID FROM app.Cars WHERE OldCarID IN {car_ids}", engine)
    return pd.read_sql("SELECT CarID, OldCarID FROM app.Cars", engine)
...
def get_order_details(engine: Engine, old_order_detail_ids: pd.Series | None = None) -> pd.DataFrame:
    if old_order_detail_ids is not None:
        order_detail_ids = (0,0) + tuple(old_order_detail_ids.values.tolist())
    ...
    return pd.read_sql("SELECT OrderDetailID, OldOrderDetailID FROM app.OrderDetails", engine)

def get_items(engine: Engine, old_item_ids : pd.Series) -> pd.DataFrame:
    item_ids = tuple(old_item_ids.values.tolist()) + (0,0)
    query = text(f"""
...
def get_categories(engine: Engine, old_cat_ids : pd.Series) -> pd.DataFrame:
    cat_ids = tuple(old_cat_ids.values.tolist()) + (0,0)
    query = text(f"""
...
def get_cities(engine: Engine) -> pd.DataFrame:
    return pd.read_sql("SELECT CountryID, CityID, OldCityID FROM app.Cities", engine)

def get_suppliers(engine: Engine) -> pd.DataFrame:
    return pd.read_sql("SELECT SupplierID, OldSupplierID FROM app.Suppliers", engine)

def get_packages(engine: Engine) -> pd.DataFrame:
    return pd.read_sql("SELECT PackageID, OldPackageID FROM app.Packages", engine)

def get_warehouses(engine: Engine) -> pd.DataFrame:
    return pd.read_sql("SELECT WarehouseID, OldStoreID FROM app.Warehouses", engine)
