...
def load(df: pd.DataFrame, user_id: int, engine: Engine):
    ...
    try:
        ...
        i = 0
        while i < len(df)/5000:
            df.iloc[5000*i:5000*(i+1)].to_sql('StockTransfers', con=engine, schema='app', if_exists='append', index=False, dtype=dtype_mapping) # type: ignore
            update_last_ingested(user_id, 'dbo.inv_StockIssue', int(df.iloc[5000*i:5000*(i+1)]['OldStockIssueID'].max()))
