...
def extract_old(engine: Engine) -> pd.DataFrame:
    ...
    query = f"SELECT LandmarkID AS OldLandmarkID, Name FROM dbo.Landmark"
...
def extract_new(engine: Engine) -> pd.DataFrame:
    ...
    query = f"SELECT LandmarkID, Name FROM app.Landmarks"
    df = pd.read_sql_query(query, engine)
    log.info(f'Extracted {len(df)} rows from app.Landmarks')
    return df
...
def join(old_data: pd.DataFrame, new_data: pd.DataFrame) -> pd.DataFrame:
    ...
    old_data['Name'] = old_data['Name'].map(lambda x: 'Gas Station' if 'Gas Station' in x else x)
...
def main(if_load:bool=True):
    ...
    if if_load:
        ...
        log.info('Landmarks are Synchronized')
