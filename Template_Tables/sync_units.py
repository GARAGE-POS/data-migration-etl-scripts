...
def extract_old(engine: Engine) -> pd.DataFrame:
    ...
    query = f"SELECT UnitID AS OldUnitID, Unit AS Name FROM dbo.Units"
    df = pd.read_sql_query(query, engine)
    logging.info(f'Extracted {len(df)} rows from dbo.Units')
    return df
...
def extract_new(engine: Engine) -> pd.DataFrame:
    ...
    df = pd.read_sql_query(query, engine)
    logging.info(f'Extracted {len(df)} rows from app.Units')
    return df
...
def join(old_data: pd.DataFrame, new_data: pd.DataFrame) -> pd.DataFrame:
    ...
    joined_data.drop_duplicates(subset='OldUnitID', inplace=True)
...
def main():
    ...
    logging.info('Units are Synchronized')
